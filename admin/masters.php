...
<script type="text/javascript" src="js/custom/tables.js"></script>
  ...
  <span id="test"></span>  
        <div class="pageheader notab">
            <h1 class="pagetitle"><?=ucwords($load->site_function->parameters($_REQUEST['name']));?> List</h1>
          
<form  method="post" >
&nbsp;&nbsp;&nbsp;&nbsp;
<button class="submit radius2" onclick="window.location='masters-add.php?name=<?=$_REQUEST['name']?>'; return false;">Add <?=ucwords($load->site_function->parameters($_REQUEST['name']));?></button>
</form>
        </div><!--pageheader-->
     ...
     <th width="20" class="head1 aligncenter"><input type="checkbox" name="checkall" class="checkall" /></th>
                    ...
                    <tbody>
<?
@$sql = $db->runQuery("SELECT * FROM ".MASTERS." where type = '".$_REQUEST['name']."' ORDER BY id DESC");
...
<a href="masters-add.php?id=<?=$view['id']?>&name=<?=$_REQUEST['name']?>" class="btn btn4 btn_search" title="view record"></a>
<!--<a href="#" id="<?=$view['id']?>" tbl="<?=MASTERS;?>" class="btn btn4 btn_trash delete" title="delete record"></a>-->
