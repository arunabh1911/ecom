...
if(@$_REQUEST['id'])
{
	$data = $db->getQuery($db->runQuery("select * from ".MASTERS." where type = '".$_REQUEST['name']."' AND id = '".$_REQUEST['id']."' "));
}
?>
...
function val()
{
    ...
    <div class="centercontent">
        ...
        <div class="pageheader notab">
            <h1 class="pagetitle">
              <? if(@$_REQUEST['id']) echo 'Edit'; else echo 'Add'; ?>
              <?=ucwords($load->site_function->parameters($_REQUEST['name']));?></h1>
...
<p>
<label><?=ucwords($load->site_function->parameters($_REQUEST['name']));?> Name</label>
<span class="field">
...
<? }?>
                  ...
                  <p class="stdformbutton">
                        ...
                        <input type="hidden" name="admin" value="masters-add" />
                        <? }else{?>
                         <button class="submit radius2">&nbsp;&nbsp;Update &nbsp;&nbsp;</button>
                        <input type="hidden" name="admin" value="masters-edit" />
