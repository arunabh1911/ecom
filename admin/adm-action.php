...
<?php		
	...
	if($h == '0'){ $row.='catId||'; ?>
		...
		<td>
			<select name="category[]" style="width:200px;" class="sett" <? if($i == '1'){?> onchange="$('.sett').val(this.value);" <? }?> >
...
if($act=='update_pages')
{
	...
	//seo
	$db->runQuery("update ".CATEGORY." set seo='".$db->clearText($_POST['meta_keywords'].'||'.$_POST['meta_description'].'||'.$_POST['meta_title'])."', status = '".$_POST['status']."' where page_id='$_POST[id]' ");
